...
class TestProcurementIntentClassification(unittest.TestCase):
    ...
    def test_commercial_followup_bare_order_unaffected(self):
        # "should we order?" (bare, no specific item) stays commercial_followup
...
class TestProcurementQueryHandler(unittest.TestCase):
    """ASK-12 follow-up: _handle_procurement_query response formatting."""
    ...
    # Test 1 — exact part with quantity
...
class TestProcurementViaDispatch(unittest.TestCase):
    """End-to-end: the full WhatsApp dispatch must not return DOCUMENT NOT UNDERSTOOD
    for procurement phrases."""
