...
def _handle_procurement_query(query: str, state: dict) -> Tuple[str, dict]:
    ...
    _min_raw = first.get("min_quantity")
    if _min_raw not in (None, "", "nan", "NaN"):
        try:
            min_qty = float(_min_raw)
    ...
    if not qty:
        ...
        min_display = int(min_qty) if min_qty == int(min_qty) else min_qty
        if qty_num <= min_qty:
            lines += ["DECISION:", "REORDER RECOMMENDED", ""]
            lines += [
                "WHY:",
                f"Onboard stock ({qty}) is at or below the minimum quantity ({min_display}). Reorder to restore stock above the minimum.",
                "",
            ]
            ...
            lines += [
                "WHY:",
                f"{pn}{desc_note} has {qty} onboard, above the minimum of {min_display}.",
                "",
            ]
            risk = "Low"
            actions = [
