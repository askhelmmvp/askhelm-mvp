...
def classify_question(question: str) -> str:
    ...
    if any(term in q for term in [
        "budget", "remaining", "over budget", "overspend", "committed", "spent",
        "cost", "costs", "change order", "financial", "finance",
        "category", "categories", "left overall", "overall budget", "most exposed"
    ]):
        return "budget"
...
- Re-run the question"""
    ...
    if "over budget" in q or "overspend" in q:
...
- Watch remaining margin before approving more spend"""
        ...
        decision = "WITHIN BUDGET" if total_remaining >= 0 else "AT RISK"
        return f"""DECISION: {decision}
