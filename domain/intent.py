...
_PROCUREMENT_QUERY_SUBSTRINGS = [
    # Explicit reorder / ordering questions with an item in mind.
    # Checked BEFORE _COMMERCIAL_FOLLOWUP_SUBSTRINGS so part-number queries
    # such as "should we order more AIK111571?" are handled by stock/procurement
    # logic instead of the document-context commercial-followup handler.
    "do we need to order",
    "do i need to order",
    "need to order more",
...
def classify_text(text: str) -> str:
    ...
    # "where is/are/do we keep" yield to compliance when the query contains
    ...
    for phrase in _STOCK_QUERY_SUBSTRINGS:
        if phrase in t:
            ...
            if phrase in ("where is ", "where are ", "where do we keep ") and _where_is_compliance:
                continue  # let compliance routing handle this
            return "stock_query"
